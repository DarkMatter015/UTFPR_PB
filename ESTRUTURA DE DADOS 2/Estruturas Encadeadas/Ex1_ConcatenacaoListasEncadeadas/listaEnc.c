...
void insereFimLista(Lista *lst, int chave) {
    //criar novo itemília que vai ser guardado na lista
    Item novo;
    novo.chave = chave;
    //...demais campos que possam existir na struct item
    //cria nova célula que vai guardar o item
...
void concatenaListas(Lista *lst1, Lista *lst2) {
    // Se a primeira lista está vazia, a concatenação é simplesmente a segunda lista
    if (verificaListaVazia(lst1)) {
        lst1->primeira = lst2->primeira;
    } else {
        // Encontrar a última célula da primeira lista
        Celula *ultima = lst1->primeira;
        while (ultima->prox != NULL) {
            ultima = ultima->prox;
        }
        // Ligar a última célula da primeira lista ao início da segunda
        ultima->prox = lst2->primeira;
    }
    // Após a concatenação, lst2->primeira deve ser NULL para evitar duplicação ao liberar
    lst2->primeira = NULL;
}
